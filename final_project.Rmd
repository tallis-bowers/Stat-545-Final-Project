...
We have our algorithm, but the problems of intermediate distributions and transition proposals remain.  Our intermediate distributions will be set as annealing between our initial proposal distribution, $f_n(x)$, and our target disbtribution, $f_0(x)$:

$f_j(x) = f_0(x)^{\beta_j}f_n(x)^{1-\beta_j}$

where $1 = \beta_0 > \beta_1 > \cdots \beta_{n-1} > \beta_n = 0.

By making a symmetric proposal such as $\mathcal{N}(0,I)$, our acceptance probability is from Metropolis-Hastings, simply:
...
The second application of AIS we will attempt to implement is
