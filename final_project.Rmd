...
anealled_transition = function(x,funct){
  for(i in 1:5){
    xnew = x+runif(0,1)
    if(runif(0,1) < funct(xnew)/funct(x)){
      x = xnew
    }
  }
  x
...
anealled_sampling = function(n,n_itter){
  for(i in 1:n){
    x = rnorm(0,1)
    w=1
    
    prop.fun = function(x,mu) dnorm(x, mu, 1)
    for(j in 1:n_itter){
      x = anealled_transition(x,prop.fun)
    }
    samples[i] = x
    weights[i] = w
  }
